...
$i=0;
$xx = "-K -Ba2f1:'Focal depth (km)':/a100f20:Misfit:WSne";
foreach $eve (@aa){
  $ii=1;
  $best=1;
  $min=1.0e+19;
  ...
  open(PLT, "| psxy -JX6/3.6 -R$dep[0]/$dep[$ii]/-100/500 $xx -X1.5" );
  ...
  printf PLT "%f 450 10 0 0 1 %s h=%4.1f %4.1f\n",$dep[0]+2,$eve,$depth,$sigma;
  close(PLT);
  $xx = "-O -K -X1 -Y2 -Ba2f1/a100f20WSne";
  $i++;
  if ($i == 5) {
    $xx = "-X3.5 -O -K -Y-8 -Ba2f1/a100f20WSne";
  }
